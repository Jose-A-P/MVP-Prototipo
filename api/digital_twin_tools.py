...
"""
Este módulo implementa todas las herramientas necesarias para operar
el Digital Twin financiero basado en escenarios. Contiene:

1. Funciones para cargar el modelo y el scaler.
2. Funciones para ejecutar escenarios de estrés financieros.
3. Mecanismos para generar y exportar datasets simulados.
4. Lógica principal para que el chatbot ejecute escenarios
   basados en lenguaje natural.
5. Utilidades para versionar escenarios mediante scenario_manager.

El Digital Twin permite simular deterioro en ingresos, tasas y
comportamiento de pago y calcular el impacto en la probabilidad de impago.
"""
...
# ============================================================
# Guardar versiones de escenarios
# ============================================================

def persist_scenario_version(df_result, scenario_type: str, income_drop=None, rate_inc=None, behavior=None, cluster=None):
    """
    Guarda una versión del escenario simulando y registra su metadata.
    Retorna el ID del escenario almacenado.

    df_result: DataFrame completo con prob_base, prob_scenario y delta_prob.
    scenario_type: tipo de escenario (global o cluster).
    income_drop, rate_inc, behavior: parámetros del escenario.
    cluster: cluster afectado si el escenario es localizado.
    """
...
def load_model_and_scaler():
    """
    Carga el modelo predictivo y el scaler utilizados para generar
    probabilidades de impago. Si cualquiera de los dos falta,
    retorna None para indicar que no existe un modelo entrenado.

    También define los nombres de las columnas que deben ser
    utilizadas como features durante la predicción.
    """
...
# ============================================================
# Simulación y exportación de escenarios
# ============================================================

def simulate_and_export(df, scenario_name, **kwargs):
    """
    Ejecuta un escenario específico sobre un dataset completo.

    Produce:
    - df_out: dataset con las columnas prob_base y prob_scenario.
    - archivos CSV necesarios para Streamlit y RAG.

    scenario_name: tipo de escenario (income, rate, behavior, combined).
    kwargs: parámetros del escenario.
    """
...
# ============================================================
# Ejecución principal de escenarios solicitados por el chatbot
# ============================================================

def chatbot_execute_action(prompt):
    """
    Interpreta el texto del usuario y ejecuta el escenario correspondiente.
    Regresa una respuesta en lenguaje natural para el chatbot.
    """
...
def load_latest_data():
    """
    Carga el archivo combinado de clientes simulados.

    Si existe, calcula delta_prob si no está presente.
